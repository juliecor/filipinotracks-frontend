...
import {
  NAVY, GOLD, INFO, SURFACE_SUBTLE, BORDER, TEXT_BODY, TEXT_MUTED,
} from '../../theme/theme'
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 0.5, gap: 1 }}>
        ...
        <ChevronRightIcon sx={{ fontSize: 18, color: isActive ? GOLD : '#CBD5E1', flexShrink: 0, mt: 0.2 }} />
      </Box>
      ...
      {(m.city_municipality || m.province) && (
        <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.3, fontSize: '0.7rem', color: TEXT_MUTED, mb: 0.8 }}>
        ...
        <Box sx={{ display: 'flex', gap: 0.6, flexWrap: 'wrap' }}>
          ...
          {!hasGeo && (
            <Chip
              label="No location"
              size="small"
              sx={{ fontSize: '0.6rem', fontWeight: 700, height: 18, bgcolor: '#F1F5F9', color: '#94A3B8' }}
            />
          )}
        </Box>
        {meta && (
